...
public class SudokuValidator {
    ...
    // Helper method to check if a group of cells contains unique digits 1-9
    // (ignoring zeros)
    ...
    // Validate the Sudoku board with custom zones
    public static boolean isValidSudoku(int[][] board, List<List<int[]>> customZones) {
        ...
        for (List<int[]> zone : customZones) {
            List<Integer> zoneCells = new ArrayList<>();
            for (int[] cell : zone) {
                int r = cell[0];
                int c = cell[1];
                zoneCells.add(board[r][c]);
            }
            if (!isValidGroup(zoneCells)) {
                return false;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Valid board with custom zones: " + isValidSudoku(validBoard, customZones)); // Expected:
                                                                                                        // true
        ...
        System.out.println("Invalid board with custom zones: " + isValidSudoku(invalidBoard, customZones)); // Expected:
                                                                                                            // false
    }
}
